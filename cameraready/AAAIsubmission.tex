...
\textbf{Evaluation Metrics}. The same set of measurements are used as in previous sections, which includes AUC, NDCG and MRR. 
\begin{table}[htp]
...
\begin{table}[htp]
\caption{AUC improvements with $b$ learnt for each user and each aspect, scale of the strength parameter $\theta$ and user-specific aspect-specific threshold parameter $\mathbf{b}_{u,k}$.}
\small
\centering
\scalebox{0.9}{
\begin{tabular}{c|c|c |c}
\hline
Dataset   &Improve($\%$)& $\sigma(\mathbf{b}_{u})$& $\theta$ \\\hline
