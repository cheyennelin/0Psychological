...
\textbf{Evaluation Metrics}.  The goal is to reconstruct the observed rankings for each user. Hence we adopt different ranking evaluation metrics, including AUC, NDCG, Precision, Recall and MRR. Precision and Recall are concepts borrowed from classifier evaluation metrics which are both based on the fraction of correctly ordered test pairs. To be specific, an item $p$ is predicted to be a winner in a pair if there exists another item $q$ and for user $u$ the prediction favors $p$, i.e. $p(p\succ_u q)> p(q\succ_u p)$. For each item $p$, precision is the fraction with the number of correctly identified winners (i.e. $p$ is predicted to be superior than $q$ for user $u$ while the actual rating $\hat\Rating_{u,p}>\hat\Rating_{u,q}$) as numerator, and the number of pairs where $p$ is predicted to be a winner as denominator. Similarly, recall is the number of correctly identified winners divided by the number of pairs where $p$ is truly a winner. The reported precision and recall are averaged over all items. 
...
A general observation in Table.~\ref{tab:rankingresult} is that embedding non-compensatory rules greatly improves existing ranking models. 


\subsection{Ranking Performance for Sessional Feedback}

\textbf{Data Sets} In our model the prominent aspect is sampled for each evaluation session. In the previous experiments, the evaluation sessions are considered to be associated with each rating. Here we evaluate model performance when the user interaction session information is available. We use two real world datasets: (1) Tmall\footnote{https://ijcai-15.org/index.php/repeat-buyers-prediction-competition} is a collection of user shopping sessions, where in each session the user has four types of activities: click, add to cart, add to favorite and purchase.  (2) Yoochoose\footnote{http://2015.recsyschallenge.com}. 
\begin{table}[htp]
\caption{Statistics of Datasets with sessional feedback}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline
...
\textbf{Comparative Methods}. We compare the NCR improved versions with the original versions on three widely adopted rating prediction methods 
...
\begin{table}[htp]
\tiny
\caption{Comparative ranking prediction performance graded sessional feedback}
\begin{center}
\begin{tabular}{|c|c|c|c|c|c|c|}
\hline
Dataset & Method & MAP & NDCG & Precision & Recall & MRR \\\hline
...
\hline
Dataset & Method & AUC & NDCG & RMSE & MAE & MRR \\\hline
\multirow{2}{*}{Movielens} & MF &  & 	 &	 &	 &	 
 \\\cline{2-7}
 & MF-NCR & & 	 &	 &	 &	 
 \\\hline
\multirow{2}{*}{Filmtrust} & MF & &	 &	 	&  &	 
 \\\cline{2-7}
 & MF-NCR &  & 	 &	 & 	 & 	 
\\\hline
 \multirow{2}{*}{CiaoDVD} & MF & & 	 &	 & 	 	 &  
\\\cline{2-7}
 & MF-NCR & & 	 &	 &	 	&  
 \\\hline
\end{tabular}
